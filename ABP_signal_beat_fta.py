...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--beats_directory", help="Directory with subfolders for identity", default="./ABP/ABP_data/beats")
    parser.add_argument("-o", "--output_folder", help="Directory where to store filtered csvs", default="./ABP/ABP_data/beats-post-FTA")
    parser.add_argument("-r", "--force_redo", action='store_true', default=False)
    parser.add_argument("-p", "--params", action='store', default="./ABP/ABP_params.yaml")
    ...
    reference = pd.read_csv("./ABP/ABP_data/beat_visuals/NOFTA.csv", index_col=False).values[..., :-1].mean(axis=0)
        ...
        for i, user in enumerate(sorted(users)):
            ...
            for file in sorted(user_files):
                ...
                if not os.path.isfile(json_out_fpath) or not os.path.isfile(img_fpath):
                    ...
                    fiducial_points = json.load(open("./ABP/ABP_data/fiducial_points/%s/%s" % (user, file), "r"))
