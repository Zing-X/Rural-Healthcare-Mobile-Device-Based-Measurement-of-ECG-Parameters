...
class SignalExtractor():
    ...
    def red_channel_mean(self, frames, **kwargs):
        ...
        print("initial_skip_seconds_front : %d"%kwargs["initial_skip_seconds_front"])
        print("initial_skip_seconds_rear : %d"%kwargs["initial_skip_seconds_rear"])

        signal = np.array(signal)
        samples_to_skip_front = kwargs["initial_skip_seconds_front"]*self.sample_rate
        signal = signal[samples_to_skip_front:]  # ignore first second because of auto exposure
        return signal
    ...
    def red_ch_threshold(self, frames, n_calib_frames=90, perc=80, **kwargs):
        ...
        signal = np.array(signal)
        signal = signal[kwargs["initial_skip_seconds_front"]*self.sample_rate:]  # ignore first second because of auto exposure
        return signal
...
def process_single_file(file, user_fold, output_folder, params, users, se, user, i):
    ...
    if not os.path.isfile(csv_fpath):
        ...
        while success:
            ...
            try:
                frame = cv2.resize(frame1, (360, 360), interpolation=cv2.INTER_LINEAR)
            except:
                print('Fail to resize (360*360)')
                break
            ioo+=1
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--video_directory", help="Directory with subfolders for identity", default="./ABP_data/videos")
    parser.add_argument("-o", "--output_folder", help="Directory where to store computed csvs", default="./ABP_data/extracted")
    ...
    parser.add_argument("-n_cpu", "--number_of_cpus", action='store', type=int, default=-1)
    parser.add_argument("-p", "--params", action='store', default="./ABP_params.yaml")
    ...
    if args.filename is None:
        ...
        for i, user in enumerate(sorted(users)):
            ...
            for file in sorted(user_files):
                ...
                df.iloc[30:].astype(float).plot(kind="line", subplots=True, figsize=(16, 4*n_extractors), layout=(n_extractors, 1), grid=True)
    ...
    if args.filename is not None:
        ...
        print("rcm")
        signal1 = se.red_channel_mean(frames=list_of_frames, initial_skip_seconds_front=0.5, initial_skip_seconds_rear=0.5)*-1

        print("ica")
        signal3 = se.ica_decomposition(frames=list_of_frames, initial_skip_seconds_front=0.5, initial_skip_seconds_rear=0.5)*-1

        print("luma")
        signal4 = se.luma_component_mean(frames=list_of_frames, initial_skip_seconds_front=0.5, initial_skip_seconds_rear=0.5)*-1
