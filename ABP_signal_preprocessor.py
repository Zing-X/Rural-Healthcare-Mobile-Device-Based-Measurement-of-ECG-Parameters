...
class SignalPreprocessor():
    ...
    def cut_end(self, signal, seconds, **kwargs):
        n_frames = self.sample_rate * seconds
        return np.concatenate((signal[:len(signal) - n_frames], np.full(n_frames, np.nan)), axis=0)
...
def visualize_signal(signals, labels, output_fname, title=""):
    ...
    l_tp = [
        "luma_mean",
        "luma_mean>cut_start>cut_end",
        "luma_mean>cut_start>cut_end>hpf",
        "luma_mean>cut_start>cut_end>hpf>bpf_bpm",
        "luma_mean>roll_avg>sub>lpf>cut_start>cut_end"
    ]
    n_plots = len(l_tp)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--signals_directory", help="Directory with subfolders for identity", default="./ABP_data/extracted")
    parser.add_argument("-o", "--output_folder", help="Directory where to store filtered csvs", default="./ABP_data/preprocessed")
    ...
    if args.filename is None:
        users = os.listdir(args.signals_directory)  # 取得檔案列表
                ...
                if not os.path.isfile(csv_fpath) or not os.path.isfile(img_fpath):
                    try:
                        ...
                        for source in params["preprocessor"]["sources"]:
                            ...
                            # check whether source is in extracted_s.columns.values
